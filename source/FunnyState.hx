...
class FunnyState extends MusicBeatState // i just stole this file from another mod I worked on.
{
	...
	override function create()
	{
		...
		DiscordClient.changePresence("In the Funny Menu", null);
		#end
		...
		var creditsList = CoolUtil.coolTextFile(Paths.txt('funnyList'));
        for (i in 0...creditsList.length)
        {
            var name:String = creditsList[i];
            var path = "assets/dawn/images/funny/" + name;
            if (FileSystem.exists(path))
			{
              ...
              //  trace("folder exists: " + path);
				...
				if (jsonc != null) 
				{
					...
					var array:Array<Dynamic> = [jsonc.photoName, jsonc.description, jsonc.link, jsonc.hexcode, xAdd, yAdd, scaleX, scaleY];
					//trace(array);
		...
		for (i in 0...creditsStuff.length)
		{
			...
			var text:Alphabet = new Alphabet(0, 15, creditsStuff[i][0], true);
			...
			if(isSelectable) 
			{
				var pic:FlxSprite = new FlxSprite(0, 200);
            	var name:String = creditsList[i];
            	pic.loadGraphic(Paths.image('funny/' + name + "/image", 'dawn'));
