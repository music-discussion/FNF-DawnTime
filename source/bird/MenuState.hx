...
class MenuState extends FlxState
{
	...
	override public function create():Void
	{
		...
		var title:FlxText = new FlxText(0, 30, FlxG.width, "Flappy Bird", 35);
		title.setFormat(null, 35, 0xFFFFFFFF, "center");
		add(title);

		var controls:FlxText = new FlxText(0, 30, FlxG.width, "Controls:\nSpace / Mouse - Flap\nEscape / Backspace - Leave\nR - Reset", 50);
		controls.setFormat(null, 35, 0xFFFFFFFF, "center");
		...
		{
			FlxG.switchState(new bird.PlayState());
		}
		choices.push(tmp);
		add(tmp);
		...
		tmp.onClick = function():Void
		{
			this.openSubState(new bird.OptionsMenu());
		}
		choices.push(tmp);
		add(tmp);
	...
	override public function update(elasped:Float):Void
	{
		...
		else if(FlxG.keys.anyPressed(["SPACE", "ENTER"]) || (mouseSelected && FlxG.mouse.pressed))
		{
			...
			return;
		} else if (FlxG.keys.anyJustPressed([FlxKey.ESCAPE, FlxKey.BACKSPACE]))
		{
